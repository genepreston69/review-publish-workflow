...
// Memoized form card component for better performance
const FormCard = React.memo(({ 
  ...
  const handleView = useCallback(() => onView(form.id), [onView, form.id]);
  const handleEdit = useCallback(() => onEdit(form.id), [onEdit, form.id]);
  const handlePublish = useCallback(() => onUpdateStatus(form.id, 'published'), [onUpdateStatus, form.id]);
  const handleUnpublish = useCallback(() => onUpdateStatus(form.id, 'draft'), [onUpdateStatus, form.id]);
  ...
  return (
    <Card className={isEditing ? 'ring-2 ring-blue-500' : ''}>
      <CardHeader>
